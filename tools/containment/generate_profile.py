...
def _header(workspaces: list[str], tmp_root: str, os_version: str | None,
            literal_ips: bool, covered_loopback: list[str],
            broker_sockets: list[str]) -> list[str]:
    ...
    if broker_sockets:
        lines += [
            ";",
            "; Broker composition: the wrapped tree may connect to these AF_UNIX policy-broker",
            "; sockets (one network-outbound unix-socket rule each, below). Without a",
            "; --broker-socket entry the broker socket is unreachable from inside the",
            "; sandbox and broker mode degrades to advisory local mode.",
        ]
    if workspaces:
        lines += [
            ";",
...
def build_profile(*, workspaces=(), allows=(), literal_ips: bool = False,
                  tmp_root: str | None = None, os_version: str | None = None,
                  broker_sockets=()) -> str:
    ...
    # Unix-socket literals match exactly what the kernel sees: resolve symlinks
    # (macOS /tmp -> /private/tmp), or a symlinked path never matches and the
    # broker stays unreachable. Sorted and deduplicated like every other input.
    sockets = sorted({os.path.realpath(p) for p in broker_sockets})
    ...
    lines = _header(write_dirs, tmp_root, os_version, literal_ips, covered, sockets)
    lines += [
        "",
        "(version 1)",
        "(deny default)",
    ...
    if sockets:
        lines.append("")
        lines.append("; policy-broker composition: AF_UNIX socket connects")
        for sock in sockets:
            lines.append(f'(allow network-outbound (remote unix-socket (path-literal "{sock}")))')
    return "\n".join(lines) + "\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--broker-socket", action="append", default=[], metavar="PATH",
                        help="policy-broker AF_UNIX socket the wrapped tree may connect to "
                             "(repeatable, sorted; absolute path) — without it the broker "
                             "socket is unreachable inside the sandbox and broker mode "
                             "degrades to advisory local mode")
    ...
    for sock in args.broker_socket:
        if not os.path.isabs(sock):
            parser.error(f"--broker-socket {sock!r} is not an absolute path — the SBPL "
                         f"unix-socket literal must name the socket exactly as the kernel sees it")
    if not literal_ips:
    ...
    text = build_profile(workspaces=args.workspace, allows=allows, literal_ips=literal_ips,
                         broker_sockets=args.broker_socket)
    if args.out:
