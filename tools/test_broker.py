...
sc = call("scope.check", workspace=ws, url="https://example.test./a")
check("scope.check strips one trailing dot before host comparison",
      sc["ok"] is True and sc["in_scope"] is True and sc["host"] == "example.test")
sc = call("scope.check", workspace=ws, url="HTTPS://EXAMPLE.TEST/a")
check("scope.check compares hosts case-insensitively (uppercase allowed)",
      sc["ok"] is True and sc["in_scope"] is True)
...
# Backlog item 12: keyed ledger chain + audit voucher verification (broker mode).
# Happy path: the broker-mode workspace audits clean — keyed event hashes verify
# with the home key, voucher sigs verify, and every attested nonce is consumed.
(_vr2 / "11_runtime/tool-registry.yaml").write_text("tools: []\n")
(_vr2 / "11_runtime/lab-status.yaml").write_text("status: UNKNOWN\n")
_sub12 = subprocess.run([sys.executable, str(TOOLS / "audit.py"), str(_vr2)],
                        capture_output=True, text=True)
check("item 12: broker-mode merges audit clean (keyed chain, sig, consumption)",
      _sub12.returncode == 0)
_keyed = [json.loads(line) for line in
          (_vr2 / "11_runtime/events.jsonl").read_text().splitlines() if line.strip()]
check("item 12: broker-mode events carry the keyed marker",
      bool(_keyed) and all(e.get("keyed") is True for e in _keyed))
# Forged vouchers: self-consistent bindings, random sigs — the audit must fail
# on the HMAC, not pass on cross-checks.
_vr12, _vc12, _ve12, _vq12 = _w1_review_root()


def _forged_packet(axis: str, reviewer: str, run: str, nonce: str) -> dict:
    pkt = {"cycle_id": "C-0001", "evidence_refs": [_ve12], "next_step": f"{axis} review",
           "review": {"axis": axis, "verdict": "pass", "reviewer": reviewer, "run_id": run,
                      "evidence_quotes": [{"evidence_ref": _ve12, "quote": _vq12}]}}
    pkt["review"]["attestation"] = {
        "workspace": str(_vr12.resolve()), "cycle_id": "C-0001", "hypothesis_id": "H-0001",
        "axis": axis, "reviewer": reviewer, "run_id": run,
        "packet_sha256": _w1_digest(pkt), "nonce": nonce,
        "expires_at": "2099-01-01T00:00:00Z", "sig": "0" * 64}
    return pkt


_vc12.append("WORKER_RESULT", "worker_result", "WR-objective", cycle_id="C-0001",
             evidence_refs=[_ve12],
             payload=_forged_packet("objective", "rev-a", "run-1", "aa" * 16))
_vc12.append("WORKER_RESULT", "worker_result", "WR-method", cycle_id="C-0001",
             evidence_refs=[_ve12],
             payload=_forged_packet("method", "rev-b", "run-2", "bb" * 16))
_vc12.transition_cycle("C-0001", "REVIEWED", reason="forged vouchers", evidence_refs=[_ve12])
_sub12f = subprocess.run([sys.executable, str(TOOLS / "audit.py"), str(_vr12)],
                         capture_output=True, text=True)
check("item 12: forged voucher signatures fail the audit",
      _sub12f.returncode != 0 and "signature does not verify" in _sub12f.stdout)
# Issued-but-never-consumed vouchers: real sigs, no merge — hand-appended
# attested packets are not merged reviews.
_vr13, _vc13, _ve13, _vq13 = _w1_review_root()


def _issued_packet(axis: str, reviewer: str, run: str) -> dict:
    pkt = {"cycle_id": "C-0001", "evidence_refs": [_ve13], "next_step": f"{axis} review",
           "review": {"axis": axis, "verdict": "pass", "reviewer": reviewer, "run_id": run,
                      "evidence_quotes": [{"evidence_ref": _ve13, "quote": _vq13}]}}
    return _w1_issue(_vr13, pkt, "H-0001")


_vc13.append("WORKER_RESULT", "worker_result", "WR-objective", cycle_id="C-0001",
             evidence_refs=[_ve13], payload=_issued_packet("objective", "rev-a", "run-1"))
_vc13.append("WORKER_RESULT", "worker_result", "WR-method", cycle_id="C-0001",
             evidence_refs=[_ve13], payload=_issued_packet("method", "rev-b", "run-2"))
_vc13.transition_cycle("C-0001", "REVIEWED", reason="unconsumed vouchers", evidence_refs=[_ve13])
_sub13 = subprocess.run([sys.executable, str(TOOLS / "audit.py"), str(_vr13)],
                        capture_output=True, text=True)
check("item 12: never-consumed vouchers fail the audit",
      _sub13.returncode != 0 and "never consumed" in _sub13.stdout)
# Keyed-chain tamper: flipping one payload byte breaks the HMAC hash.
_vr14, _vc14 = fixture_root()
_vc14.append("NOTE", "note", "N-1", reason="sealed note for tamper check")
_ledger14 = _vr14 / "11_runtime/events.jsonl"
_ledger14.write_text(_ledger14.read_text().replace("sealed note", "sealed NOTE"))
_sub14 = subprocess.run([sys.executable, str(TOOLS / "audit.py"), str(_vr14)],
                        capture_output=True, text=True)
check("item 12: a tampered keyed event fails the audit",
      _sub14.returncode != 0 and "event_hash mismatch" in _sub14.stdout
      and "keyed" in _sub14.stdout)
# Cross-workspace voucher consume stays refused at the broker.
_vrx, _vcx = fixture_root()
_voucher_x = call("review.issue", workspace=str(_vrx.resolve()), cycle_id="C-0001",
                  hypothesis_id="H-0001", axis="objective", reviewer="rev-a", run_id="run-1",
                  packet_sha256="ab" * 32)["voucher"]
_vother, _ = fixture_root()
_refused_x = call("review.consume", workspace=str(_vother.resolve()), voucher=_voucher_x)
check("item 12: cross-workspace voucher consume is refused",
      _refused_x.get("ok") is False and "different workspace" in str(_refused_x.get("error")))
