...
- Line counts below are approximate at tag `v7.2` (post-review hardening release).
...
| `AGENTS.md` | 146 | The operational flow an agent must follow |
| `tools/control_plane.py` | 1781 | **The single policy seam**: event schema, state machine, projections, guards, audit/freshness/evidence/technique records |
| `tools/audit.py` | 566 | What the machine audit actually checks (and what it does not) |
...
| `tools/test_control_plane.py` | 1028 | Executable spec for the seam |
...
| `dsh-plugin/executor.integration.test.mjs` | 234 | Executable spec for the executors |
...
`13_RUNTIME.md` (61) · `15_TOOLING.md` (437) · `16_RESEARCH_LANES.md` (33) ·
