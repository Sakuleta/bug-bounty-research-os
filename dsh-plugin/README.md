...
Workspace detection: walking up from the session cwd for `OS_VERSION` plus at least one of
`11_runtime/events.jsonl`, `00_control/engagement.yaml` or the `11_runtime/` directory — a
deleted ledger alone does not disarm the enforcer. Non-OS workspaces are untouched. Scope
